...
`QueryClient` をコンポーネントの外で `const queryClient = new QueryClient()` と書くと、モジュールが読み込まれたタイミングで1度だけ生成された**シングルトン**になります。サーバーサイドではこのインスタンスが全リクエストで共有されるため、ユーザーAのキャッシュがユーザーBのレスポンスに混入するデータ漏えいが起こります。`useState(() => new QueryClient())` にしておくと、リクエストごとに独立したインスタンスが生成されるため、ユーザー間でキャッシュが分離されます。
